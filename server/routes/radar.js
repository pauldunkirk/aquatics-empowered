...
const columnNames = '(latitude, longitude, place_id, coord_list, keyword)';
...
router.get('/', function(req, res) {
  ...
  pool.connect(function(err, client, done) {
    ...
    client.query('SELECT * FROM google_radar_results;', function(err, result) {
      ...
      console.log('****************************************************** got radar results',result.rows);
    });
  });
});
...
router.post('/', function(req, res) {
  ...
  console.log('RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR radar results incl place_id', facil);
  pool.connect( function(err, client, done) {
    err && res.sendStatus(503);
    client.query(
      'INSERT INTO google_radar_results ' + columnNames + ' VALUES ($1, $2, $3, $4, $5)',
      [facil.latitude, facil.longitude, facil.place_id, facil.coord_list, facil.keyword],
      function(err) {
        done();
